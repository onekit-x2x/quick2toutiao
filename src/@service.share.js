...
module.exports = {
  ...
  share(quick_object) {
    ...
    if (quick_platforms) {
      ...
      for (const platforms of quick_platforms) {
        ...
        if (['toutiao_CIRCLE', 'WEIBO', 'QQ', 'SYSTEM'].indexOf(platforms) > 0) {
          quick_temp.push(platforms)
        }
      }

      if (quick_temp.lenght > 0) {
        console.warn("[quick2toutiao] only support 'toutiao'")
      }
    }
    ...
    if (flag === 0) {
      ...
      if (quick_imagePath) {
        tt_object.imageUrl = quick_imagePath
      }
    } else if (flag === 1) {
      // 纯图片
      tt_object.imageUrl = quick_imagePath
      ...
      // 其它类型暂不支持
      if (quick_fail) {
        quick_fail("[quick2toutiao] only support 'quick_shareType: 0 or 2'", 203)
      }
      return
    }
    const tt_callback = {}
    ...
    if (quick_cancel) {
      tt_callback.cancel = quick_cancel
    }
  ...
  getAvailablePlatforms(quick_object) {
    ...
    // //////////////////////////////////////
    const result = ['toutiao'] // , "toutiao_CIRCLE"
    if (quick_success) {
      quick_success({
        platforms: result
      })
