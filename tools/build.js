...
const checkTtss = require('./checkttss')
const _ = require('./utils')

const ttssConfig = config.ttss || {}
 ...
 * 获取 ttss 流
 */
function ttss(ttssFileList) {
  if (!ttssFileList.length) return false

  // 加上要忽略的列表
  if (config.ignore) {
    ttssFileList = ttssFileList.concat(config.ignore)
  }
  return gulp.src(ttssFileList, {cwd: srcPath, base: srcPath})
    .pipe(checkTtss.start()) // 开始处理 import
    // .pipe(gulpif(ttssConfig.less && ttssConfig.sourcemap, sourcemaps.init()))
    // .pipe(gulpif(ttssConfig.less, less({paths: [srcPath]})))
    .pipe(checkTtss.end()) // 结束处理 import
    .pipe(rename({extname: '.ttss'}))
    .pipe(gulpif(ttssConfig.less && ttssConfig.sourcemap, sourcemaps.write('./')))
    .pipe(_.logger(ttssConfig.less ? 'generate' : undefined))
...
function buildLess(lessFileList) {
  ...
  return gulp.src(lessFileList, {cwd: srcPath, base: srcPath})
    // .pipe(checkTtss.start()) // 开始处理 import
    .pipe(gulpif(ttssConfig.less && ttssConfig.sourcemap, sourcemaps.init()))
    .pipe(gulpif(ttssConfig.less, less({paths: [srcPath], compress: true})))
    // .pipe(checkTtss.end()) // 结束处理 import
    .pipe(rename({extname: '.ttss'}))
    .pipe(gulpif(ttssConfig.less && ttssConfig.sourcemap, sourcemaps.write('./')))
    .pipe(_.logger(ttssConfig.less ? 'generate' : undefined))
...
class BuildTask {
  ...
  init() {
     ...
     * 拷贝 ttml 文件到目标目录
     */
    gulp.task(`${id}-component-ttml`, done => {
      const ttmlFileList = this.componentListMap.ttmlFileList

      if (ttmlFileList &&
        ttmlFileList.length &&
        !_.compareArray(this.cachedComponentListMap.ttmlFileList, ttmlFileList)) {
        return copy(ttmlFileList)
      }
     ...
     * 生成 ttss 文件到目标目录
     */
    gulp.task(`${id}-component-ttss`, done => {
      const ttssFileList = this.componentListMap.ttssFileList
      // console.log('ttssFileList file list', ttssFileList)
      if (ttssFileList &&
        ttssFileList.length &&
        !_.compareArray(this.cachedComponentListMap.ttssFileList, ttssFileList)) {
        return ttss(ttssFileList, srcPath, distPath)
      }
    ...
    gulp.task(`${id}-copy`, gulp.parallel(done => {
      ...
      const copyFileList = copyList.map(dir => path.join(dir, '**/*.!(ttss)'))
      ...
      if (copyFileList.length) return ttss(copyFileList, srcPath, distPath)
     ...
     * 监听 json 变化
     */
    gulp.task(`${id}-watch-json`, () => gulp.watch(this.componentListMap.jsonFileList, {cwd: srcPath, base: srcPath}, gulp.series(`${id}-component-check`, gulp.parallel(`${id}-component-ttml`, `${id}-component-ttss`, `${id}-component-js`, `${id}-component-json`))))

    /**
     * 监听 ttml 变化
     */
    gulp.task(`${id}-watch-ttml`, () => {
      this.cachedComponentListMap.ttmlFileList = null
      return gulp.watch(this.componentListMap.ttmlFileList, {cwd: srcPath, base: srcPath}, gulp.series(`${id}-component-ttml`))
    })

    /**
     * 监听 ttss 变化
     */
    gulp.task(`${id}-watch-ttss`, () => {
      this.cachedComponentListMap.ttssFileList = null
      return gulp.watch('**/*.ttss', {cwd: srcPath, base: srcPath}, gulp.series(`${id}-component-ttss`))
    })
    /**
     * 监听 less 变化
     */
    ...
    gulp.task(`${id}-build`, gulp.series(`${id}-clean-dist`, `${id}-component-check`, gulp.parallel(`${id}-component-ttml`, `${id}-component-js`, `${id}-component-less`, `${id}-component-ttss`, `${id}-component-json`, `${id}-copy`, `${id}-package-json`)))

    gulp.task(`${id}-watch`, gulp.series(`${id}-build`, `${id}-demo`, `${id}-install`, gulp.parallel(`${id}-watch-ttml`, `${id}-watch-ttss`, `${id}-watch-json`, `${id}-watch-copy`, `${id}-watch-install`, `${id}-watch-demo`, `${id}-watch-less`, `${id}-watch-ts`)))
