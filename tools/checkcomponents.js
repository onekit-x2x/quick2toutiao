...
async function checkIncludedComponents(jsonPath, componentListMap) {
  ...
  // 进入存储
  let exists = await _.checkFileExists(path.join(dirPath, `${fileName}.ttml`))
  if (exists) {
    componentListMap.ttmlFileList.push(`${fileBase}.ttml`)
  }
  exists = await _.checkFileExists(path.join(dirPath, `${fileName}.ttss`))
  exists && componentListMap.ttssFileList.push(`${fileBase}.ttss`)
